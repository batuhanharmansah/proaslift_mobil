// 💰 RECEIVABLES SCREEN
// Alacaklar Yönetim Ekranı - Liste, ekleme, ödeme alma
...
import { Receivable, AccountType, Building } from '../../types';
...
const ReceivablesScreen: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    payment_type: 'tek_sefer' as 'tek_sefer' | 'taksitli',
    installment_count: '1',
  ...
  const fetchReceivables = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get(`${API_ENDPOINTS.FINANCIAL_RECEIVABLES}?${params.toString()}`);

      // API client returns response.data, so response is already { success: true, data: {...} }
      if (response?.success && response?.data) {
        const receivablesData = response.data;
        // Check if data is paginated (has data property) or direct array
        const receivablesList = receivablesData?.data || receivablesData;
        
        if (Array.isArray(receivablesList)) {
          if (pagination.current_page === 1) {
            setReceivables(receivablesList);
          } else {
            setReceivables((prev) => [...prev, ...receivablesList]);
          }
        }
        
        // Update pagination if available
        if (receivablesData?.current_page) {
          setPagination({
            current_page: receivablesData.current_page,
            last_page: receivablesData.last_page || 1,
            per_page: receivablesData.per_page || 20,
          });
        }
        ...
        if (pagination.current_page === 1) {
          ...
          setReceivables((prev) => [...prev, ...response]);
        }
        ...
        if (pagination.current_page === 1) {
          ...
          setReceivables((prev) => [...prev, ...response.data]);
        }
      }
    } catch (error: any) {
      console.error('Fetch receivables error:', error);
      Alert.alert('Hata', 'Alacaklar yüklenemedi: ' + (error.message || 'Bilinmeyen hata'));
    } finally {
      setLoading(false);
    }
  ...
  const handleAddReceivable = useCallback(async () => {
    if (!formData.title.trim() || !formData.total_amount || !formData.building_id) {
      Alert.alert('Eksik Bilgi', 'Lütfen başlık, tutar ve bina seçin.');
      return;
    }
    ...
    try {
      ...
      const response = await apiClient.post<{ success: boolean; message: string }>(
        API_ENDPOINTS.FINANCIAL_RECEIVABLES,
        {
          ...
          installment_count:
            formData.payment_type === 'taksitli' ? parseInt(formData.installment_count) : 1,
      ...
      if (response?.success) {
        Alert.alert('Başarılı', response?.message || 'Alacak oluşturuldu');
      ...
      Alert.alert('Hata', error.response?.data?.message || 'Alacak oluşturulamadı');
    } finally {
      setLoading(false);
    }
  }, [formData, fetchReceivables, pagination]);

  const handleReceivePayment = useCallback(async () => {
    if (!paymentForm.amount || !paymentForm.account_id || !selectedReceivable) {
      Alert.alert('Eksik Bilgi', 'Lütfen ödeme tutarı ve hesap seçin.');
      return;
    }

    const paymentAmount = parseFloat(paymentForm.amount);
    if (paymentAmount > (selectedReceivable.remaining_amount || 0)) {
    ...
    try {
      ...
      const response = await apiClient.post<{ success: boolean; message: string }>(
        API_ENDPOINTS.FINANCIAL_RECEIVE_PAYMENT,
        {
  ...
  }, [paymentForm, selectedReceivable, fetchReceivables, pagination]);

  const handleDeleteReceivable = useCallback((receivable: Receivable) => {
    Alert.alert(
      'Alacağı Sil',
      `${receivable.title} alacağını silmek istediğinizden emin misiniz?`,
      [
          ...
          onPress: async () => {
            try {
              ...
              const response = await apiClient.delete<{ success: boolean; message: string }>(
                `${API_ENDPOINTS.FINANCIAL_RECEIVABLES}/${receivable.id}`
              );

              if (response?.success) {
                Alert.alert('Başarılı', response?.message || 'Alacak silindi');
                setPagination({ ...pagination, current_page: 1 });
                await fetchReceivables();
              }
            } catch (error: any) {
              console.error('Delete receivable error:', error);
              Alert.alert('Hata', error.response?.data?.message || 'Alacak silinemedi');
            } finally {
              setLoading(false);
            }
          },
    ...
    <TouchableOpacity style={styles.receivableCard}>
      <View style={styles.receivableHeader}>
        <View style={styles.receivableInfo}>
          ...
          <View style={styles.receivableMeta}>
            ...
            {item.building && (
              <Text style={styles.receivableBuilding}>• {item.building.name}</Text>
            )}
          </View>
        </View>
        ...
        <View style={styles.progressBar}>
          <View
            style={[
              styles.progressFill,
              {
                width: `${(item.received_amount / item.total_amount) * 100}%`,
                backgroundColor: item.status === 'tamamlandi' ? COLORS.success[500] : COLORS.primary[500],
              },
            ]}
          />
        </View>
        <View style={styles.progressText}>
          <Text style={styles.progressLabel}>Tahsil Edilen</Text>
          <Text style={styles.progressValue}>
            {formatCurrency(item.received_amount)} / {formatCurrency(item.total_amount)}
          </Text>
        </View>
      ...
      {item.remaining_amount > 0 && (
        <View style={styles.receivableActions}>
          ...
          >
            <Ionicons name="cash-outline" size={18} color={COLORS.success[600]} />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Ionicons name="arrow-down-circle-outline" size={64} color={COLORS.gray[400]} />
            <Text style={styles.emptyText}>Henüz alacak bulunmuyor</Text>
            <Text style={styles.emptySubtext}>Yeni alacak eklemek için + butonuna tıklayın</Text>
          </View>
        }
        ListFooterComponent={
                ...
                <View style={styles.inputGroup}>
                  ...
                  <View style={styles.typeButtons}>
                    <TouchableOpacity
                      style={[
                        styles.typeButton,
                        formData.payment_type === 'tek_sefer' && styles.typeButtonActive,
                      ]}
                      onPress={() => setFormData({ ...formData, payment_type: 'tek_sefer' })}
                    >
                      <Text
                        style={[
                          styles.typeButtonText,
                          formData.payment_type === 'tek_sefer' && styles.typeButtonTextActive,
                        ]}
                      >
                        Tek Sefer
                      </Text>
                    ...
                    <TouchableOpacity
                      style={[
                        styles.typeButton,
                        formData.payment_type === 'taksitli' && styles.typeButtonActive,
                      ]}
                      onPress={() => setFormData({ ...formData, payment_type: 'taksitli' })}
                    >
                      <Text
                        style={[
                          styles.typeButtonText,
                          formData.payment_type === 'taksitli' && styles.typeButtonTextActive,
                        ]}
                      >
                        Taksitli
                      </Text>
                  ...
                  {formData.payment_type === 'taksitli' && (
                    <TextInput
                      style={styles.input}
                      placeholder="Taksit sayısı"
                      keyboardType="number-pad"
                      value={formData.installment_count}
                      onChangeText={(text) => setFormData({ ...formData, installment_count: text })}
                    />
                  )}
                </View>
            ...
            {selectedReceivable && (
              <View style={styles.formContainer}>
                <View style={styles.paymentInfo}>
                  <Text style={styles.paymentInfoLabel}>Alacak:</Text>
                  <Text style={styles.paymentInfoValue}>{selectedReceivable.title}</Text>
                </View>
                <View style={styles.paymentInfo}>
                  <Text style={styles.paymentInfoLabel}>Kalan Tutar:</Text>
                  <Text style={[styles.paymentInfoValue, styles.paymentInfoAmount]}>
                    {formatCurrency(selectedReceivable.remaining_amount)}
                  </Text>
                </View>
            ...
            <View style={styles.modalActions}>
              ...
              >
                {loading ? (
                  ...
                  <Text style={styles.saveButtonText}>Ödeme Al</Text>
                )}
              </TouchableOpacity>
            </View>
          </View>
